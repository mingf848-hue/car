...
- 监控 `SMART_WALLETS` 里的 Polymarket 钱包 activity。
- 首次启动默认只做 warm-up，不追历史单。
- 跟 BUY 开仓，SELL 自动平掉该 token 的本地跟单持仓。
- 每次 BUY 固定 `COPY_AMOUNT_USDC=5`。
- 首页就是可交互 Dashboard：状态、风险控制、持仓、事件、钱包评分、手动扫描。
- 体育市场筛选：Gamma 市场字段 -> 体育关键词 -> DeepSeek 兜底分类。
- 下单前检查当前 BUY/SELL 价格，超过 `MAX_SLIPPAGE_BPS` 自动跳过。
- SQLite 去重、冷却时间、每日 live 最大金额控制。
...
app/
  config.py             环境变量配置
  engine.py             跟单决策核心
  executor.py           dry-run / live CLOB 下单适配器
...
3. 构建方式选择 Dockerfile。
4. Region 建议优先选 `Tokyo`。如果你用 BYO server 并且可选云区域，`eu-west-1` 也是 Polymarket 官方文档提到的低延迟候选。
...
`SELL_MODE=close_full_on_leader_sell` 是当前自动跟卖规则：只要聪明钱包对某个 token 发出 SELL，本系统会检查自己是否曾经跟买并还有 open shares；如果有，就卖出这部分持仓。没有本地持仓时不会裸卖。
...
CI 不需要真实 Polymarket 密钥，也不会发真实订单。
