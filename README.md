...
- 首页是手机 App 式控制台：本地估算收益、余额状态、跟单投入、持仓、AI 推荐和底部导航。
- “AI 推荐”会从 Polymarket 体育排行榜读取地址，结合盈利、成交、活跃度、体育占比和近期下注推荐钱包；配置 `DEEPSEEK_API_KEY` 后会生成中文推荐理由和风险点，没有 key 时使用规则评分兜底。
...
控制台包含运行模式、自动跟单状态、本地估算收益、余额状态、AI 推荐、下注详情、跟单/暂停、本地持仓、事件流水和手动补查。
...
```bash
curl https://你的域名/portfolio
```

### 获取 AI 推荐钱包

```bash
curl https://你的域名/recommendations
```
